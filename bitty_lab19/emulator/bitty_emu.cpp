...
void BittyEmulator::EvaluateMemoryInstr()
{

    uint16_t inst = memory_[pc_];
    // std::cout << "Evaluating memory..." << std::hex << inst << std::dec << std::endl;
    if (inst % 4 != 3)
    {
        return;
    }
    uint8_t ls = (inst >> 2) & 0b1;
    if (ls)
    {
        memory_[(inst >> 10) & 0x7] = registers_[(inst >> 13) & 0x7];
        // std::cout << "Memory Evaluated " << std::hex << memory_[(inst >> 10) & 0x7] << std::dec << "Is stored" << std::endl;
    }
    else
    {
        registers_[(inst >> 13) & 0x7] = memory_[(inst >> 10) & 0x7];
        // std::cout << "Memory Evaluated " << std::hex << registers_[(inst >> 13) & 0x7] << std::dec << "Is loaded" << std::endl;
    }
...
uint16_t BittyEmulator::Step()
{
    ...
    uint16_t inst = memory_[pc_];
    if (current_inst_format_type == 0b11)
    {
        EvaluateMemoryInstr();
    }
    else
