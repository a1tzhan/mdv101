...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < NUM_TESTS; i++)
    {
        ...
        uint8_t alu_sel = (instruction >> 3) & 0xF;
        uint8_t alu_mode = (instruction >> 2) & 0x1;
        ...
        {
            errors++;
            std::cout << "alu_sel is not instruction[6:3]" << std::endl;
        }
        if (tb->alu_mode != alu_mode)
        {
            errors++;
            std::cout << "alu_mode is not instruction[2]" << std::endl;
        }
