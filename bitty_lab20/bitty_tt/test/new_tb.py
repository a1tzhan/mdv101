...
async def uart_module_test(dut):
    ...
    async def timeout_timer():
        ...
        await Timer(2 * 60 * 1e9, units="ns")  # 10 minutes in nanoseconds
    ...
    try:
        ...
        with open(log_file, "w") as log:
            while pc < len(instruction_set)-1 or pc < 255 or i < 255:
                ...
                try:
                    if(rx_register == 0):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg0.d_out.value)
                    elif(rx_register == 1):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg1.d_out.value)
                    elif(rx_register == 2):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg2.d_out.value)
                    elif(rx_register == 3):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg3.d_out.value)
                    elif(rx_register == 4):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg4.d_out.value)
                    elif(rx_register == 5):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg5.d_out.value)
                    elif(rx_register == 6):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg6.d_out.value)
                    elif(rx_register == 7):
                        dut_rx_value = int(dut.user_project.bitty_inst.reg7.d_out.value)
                    else:
                        dut_rx_value = 0
