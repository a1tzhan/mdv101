...
public:
    ...
    std::vector<uint16_t> assemble(const std::string &input_file)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            // Encode instruction
            uint16_t encoded = (inst.format == R_TYPE) ? encode_r_type(inst) : encode_i_type(inst);
...
public:
    ...
    std::vector<std::string> disassemble(const std::string &input_file)
    {
        ...
        while (std::getline(file, line))
        {
            uint16_t instruction = static_cast<uint16_t>(std::stoul(line, nullptr, 16));
            ...
            if (format == R_TYPE)
            {
                ...
                decoded = opcode_map_r[static_cast<AluOp>(alu_op)] + " " + register_map[rd] + " " + register_map[rs];
            }
            else
            {
                int8_t imm8 = static_cast<int8_t>((instruction >> 5) & 0xFF);
                decoded = opcode_map_i[static_cast<AluOp>(alu_op)] + " " + register_map[rd] + " " + std::to_string(static_cast<int>(imm8));
            }

            assembly.push_back(decoded);
        }
...
int main(int argc, char *argv[])
{
    ...
    std::string output_file = "out.txt"; // default output file
    bool assemble = false;
    ...
    try
    {
        if (assemble)
        {
            ...
            {
                out << std::hex << std::setw(4) << std::setfill('0') << instruction << std::dec << "\n";
            }
        }
        else
        {
